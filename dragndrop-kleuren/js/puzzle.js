...
class PuzzleSlot {
    constructor (boardX, boardY, boxSize, boardBounds, targetPiece, id) {
        this.boardX = boardX;
        this.boardY = boardY;
    ...
    makeCorrectSound() {
	var audio = document.getElementById("audio_goed"+this.id);
        if (audio) audio.play();
    }

    makeIncorrectSound() {
	var audio = document.getElementById("audio_fout"+this.id);
        if (audio) audio.play();
    }
...
function audio_callback(index) {
  return function(e) {
    var audio = document.getElementById("audio0"+index);
    if (audio) audio.play();
  };
}

function audio_testcallback() {
...
const gameBoard = {
    ...
    init: function () {
        ...
        this.image.src = 'images/kleuren.png';
    },
    ...
    initPieces: function () {
        ...
        while ( col * this.pieceSize < this.image.width && row * this.pieceSize < this.image.height ) {
            ...
            this.slots.push(new PuzzleSlot(imgX, imgY, this.pieceSize, this.playingBoard, puzzlePiece, counter));
    ...
    drawPuzzleOutline: function () {
	    ...
	    var ctx = this.context; //todo

            var loudspeaker = document.getElementById("loudspeaker"+j); 
          
            this.context.strokeRect(s.left, s.top, s.width, s.height);

	    this.context.drawImage(loudspeaker,0,0,225,225,s.left, s.top, s.width, s.height); 
        }
    },
    ...
    onMouseUp: function (e) {
        ...
        if ( this.isDragging ) {
			...
			slot.makeCorrectSound();
                        this.unhighlightPiece(this.highlightedPiece);
                    } else {
			slot.makeIncorrectSound();
                    }
