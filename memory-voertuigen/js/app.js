...
*/ 
  ...
  shuffledCards.map(function(val) {
    //getting deck innerHTML and adding a new innerHTML per iteration to select image
      var cardName = val.slice(0,val.lastIndexOf("."));
    deck.innerHTML =
      deck.innerHTML +
      `<li class='card' id=${cardName}>
...
function flipCard(e) {
  //e.target to select the element the mouse cursor is pointing at
    console.log(e);
    console.log("audio_"+e.target.id);
    var audio = document.getElementById("audio_"+e.target.id);
    if (audio) audio.play();
...
function gameComplete() {
  ...
  timer[1].innerHTML = 'Tijd: ' + timer[0].innerHTML;
